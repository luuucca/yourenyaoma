...
const STRIP = [
  ...
  { img: 'tools.png', label: '找师傅', href: '/pros' },
  { img: 'buddies-icon.png', label: '找搭子', href: '/buddies' },
] as const
...
export default function CategoryGrid() {
  return (
    <section className="max-w-[1360px] mx-auto px-4 md:px-16 pt-3 pb-12 flex items-center justify-between gap-4 overflow-x-auto no-scrollbar">
      {STRIP.map((item, i) => {
        // last two (找师傅/找搭子) get the yellow-tinted background per design
        const accent = i >= STRIP.length - 2
        return (
            ...
            >
              <div
                className={
                  ...
                  (accent
                    ? 'bg-brand-yellow-soft border border-brand-yellow-line group-hover:border-brand-yellow'
                    : 'bg-brand-cream border border-brand-line group-hover:border-brand-ink')
                }
              >
